...
def _review_phase(wf_id="wf_review_test"):
    ...
    append_event("orchestrator", "phase_entered", data={"phase": "review", "order": 1})
...
class TestAllQaVerdictsApproved:
    ...
    def test_verdict_approved_with_reservations_is_met(self, phase_env):
        _review_phase()
        _review_task("review_dev_tc_001")
    ...
    def test_all_approved_with_reservations_is_met(self, phase_env):
        _review_phase()
        for i in range(1, 4):
            _review_task(f"review_dev_tc_{i:03d}")
            _complete_review(f"review_dev_tc_{i:03d}", phase_env,
