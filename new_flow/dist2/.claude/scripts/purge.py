...
Optional targets (explicit flags required):
    --blobs                       Orphan blobs via gc logic (or all if --reset-log)
...
Usage:
    ...
    # Nuclear: archive log, wipe everything, reset to clean state
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Operator identity (required with --reset-log)."
    )
    parser.add_argument(
    ...
    # Validation
    if args.reset_log and not args.operator:
        msg = {"error": "operator_required", "detail": "--reset-log requires --operator <identity>"}
    ...
    if not args.confirm:
        ...
        if args.as_json:
            ...
            if not plan and not log_reset_plan:
