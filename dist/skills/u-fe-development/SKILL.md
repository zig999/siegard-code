...
> **Validation rule:** before executing any step that writes to `{SESSIONS_DIR}/{SESSION}/` or reads from `{SPECS_DIR}/`, confirm the variables are resolved strings (not literal placeholders). If they are still placeholders, emit `status: blocked / reason: unresolved_context_variables` and halt.
...
- [ ] If there are backend issues: `tc-XX-backend-pending-items.md` report generated and Orchestrator notified
