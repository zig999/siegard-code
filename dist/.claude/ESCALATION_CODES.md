...
| `E28_no_supervisor_lease` | info | orchestrator (meta) | No supervisor activity within the lease TTL at phase entry (`check_supervisor_lease.py`). Advisory only — never blocks entry, because in attended mode the operator IS the supervisor | None required. To have a stall resume in minutes rather than at the next session: `/loop 5m /u-supervise <workflow_id>` (attended) or `/schedule /u-supervise <workflow_id>` (unattended) |
| `E26_workflow_left_unattended` | warning | recovery-tick (SessionStart) | Tasks remain non-terminal with no orchestrator driving them. `recovery_tick.py` promoted every due retry and reaped every silent worker it could, but continuing the phase needs an orchestrator. Liveness-gated since v2.36.0: suppressed while the log shows activity within `recovery_policy.quiet_seconds` (default 300s) — a workflow actively driven by ANOTHER session is not unattended | Re-invoke the meta-orchestrator to resume; or start supervision (`/loop 5m /u-supervise <workflow_id>`) so a stall resumes within minutes instead of at the next session |
| `E27_retry_scheduled_without_actuator` | warning | stale-monitor (`on_stop.py`) | The session ended after reaping stale tasks and scheduling their retries. Only a phase orchestrator's dispatch loop promotes a scheduled retry, and none is running — measured: retry scheduled for 23:20:02, log ends at 23:19:36 | None required: the SessionStart hook promotes them next session. To resume sooner, re-invoke the meta-orchestrator or run supervision |
| `E24_spawn_budget_exhausted` | warning (low) / critical (exhausted) | orchestrator-sdd, orchestrator-dev, orchestrator-review, orchestrator-test | Per-session subagent spawn budget is nearly or fully spent (`check_spawn_budget.py`, exit 3 / 4). Once exhausted, every further dispatch fails identically — a session-level stop condition, not a task failure | `warning`: finish the in-flight batch, then re-invoke in a fresh session — the log resumes where it stopped. `critical`: re-invoke in a fresh session; no task state needs repair, nothing was claimed. To spend less next time, scope the change with `/u-improve` instead of fanning out over every domain |
| `E25_unprovenanced_artifact` | critical | orchestrator-dev (handoff consumption gate) | The handoff manifest failed provenance validation (`PROV-*` rules in `u-handoff-validator`): a pinned artifact hash matches neither a worker-notarized `task_completed` hash nor the workflow's adoption baseline — the file was modified outside the pipeline — or the manifest on disk is not the generator's output. Integrity gates (HDF-020/021) cannot catch this; only the log notary can | Re-adopt the change through the pipeline: `/u-improve <workflow_id>` routes it via triage → writer → reviewer → validator and re-notarizes. If a human made the edit deliberately, re-run `/u-spec` so the tree is re-validated and re-baselined. Never hand-edit the log |
