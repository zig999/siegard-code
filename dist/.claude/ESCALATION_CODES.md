...
| `E26_workflow_left_unattended` | warning | recovery-tick (SessionStart) | Tasks remain non-terminal with no orchestrator driving them. `recovery_tick.py` promoted every due retry and reaped every silent worker it could, but continuing the phase needs an orchestrator | Re-invoke the meta-orchestrator to resume; or start supervision (`/loop 5m /u-supervise <workflow_id>`) so a stall resumes within minutes instead of at the next session |
