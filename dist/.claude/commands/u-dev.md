...
|-----------------|--------|
| `phase_advanced` | Show a one-line status update to the human (`Phase {completed_phase} complete → entering {next_phase}`), then apply **§Detach decision** using the report's `supervision` field. |
