...
- **2nd+ Story in the same Epic** for the same agent
- **1st Story in a new Epic** when the agent was already activated in a previous Epic during the same session
- **Post-QA correction (rework):** the agent has already processed the Story once — always use short mode
...
- Extract only the relevant Story block (not the entire `backlog.md`)
- Omit spec sections already extracted in previous activations of the same Story (rework)
