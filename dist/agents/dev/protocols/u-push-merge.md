...
### After QA approves a Story

1. Verify that the branch `feat/US-XX` (or `fix/`, `refactor/`) exists and has local commits
...
3. Ask the human if they want to **squash merge** into `main`:
   ...
   1. Squash merge — consolidate into a single commit: "feat(US-XX): [Story title]"
...
5. Remove the Developer's worktree:
   ```
   git -C "$REPO_ROOT" worktree remove "$REPO_ROOT/.claude/worktrees/US-XX"
   ```
