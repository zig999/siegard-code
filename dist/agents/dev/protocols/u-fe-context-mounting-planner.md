...
> **Instruction to the Planner (Spec-first):** "Approved specs available. Use UCs as the basis for Stories — each Story must reference UC-NN in the 'Technical notes' section. Use navigation flows (.flow.md) as reference for user journeys. Use the glossary for naming. Dependencies between domains must be reflected as dependencies between Epics/Stories."
