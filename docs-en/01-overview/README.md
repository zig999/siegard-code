...
| **Dev** | 9 agents | Code implementation (frontend and backend) |
...
## Automatic frontend/backend routing

The `/u-dev` command automatically routes to the frontend or backend pipeline based on the `domain:` field in the target project's `CLAUDE.md`. No manual specification is needed -- the orchestrator reads the configuration and selects the appropriate agents.
