...
def fresh() -> OrchState:
    ...
    apply_event(state, _evt(EventType.PHASE_ENTERED, data={"phase": "dev", "order": 1, "workflow_id": "wf-fix"}))
    return state
...
class TestPhaseEnteredError:
    ...
    def test_phase_entered_while_another_active_raises(self):
        ...
        apply_event(state, _evt(EventType.PHASE_ENTERED, data={"phase": "sdd", "order": 1, "workflow_id": "wf-fix"}))
        ...
        with pytest.raises(IllegalTransition, match="already active"):
            apply_event(state, _evt(EventType.PHASE_ENTERED, data={"phase": "dev", "order": 2, "workflow_id": "wf-fix"}))
