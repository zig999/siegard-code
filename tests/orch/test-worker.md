...
ORCH_WORKER_ID="$ORCH_WORKER_ID" python3 .claude/skills/orch-report/scripts/emit.py \
  ...
  --task-id "$ORCH_TASK_ID" \
  --attempt "$ORCH_ATTEMPT" \
  --data '{"phase":"test","note":"output file created"}'
```
...
ORCH_WORKER_ID="$ORCH_WORKER_ID" python3 .claude/skills/orch-report/scripts/emit.py \
  ...
  --task-id "$ORCH_TASK_ID" \
  --attempt "$ORCH_ATTEMPT" \
  --data "{\"phase\":\"test\",\"artifacts\":[\".orch/test-output/${ORCH_TASK_ID}-attempt${ORCH_ATTEMPT}.txt\"],\"summary\":\"test worker completed task ${ORCH_TASK_ID} attempt ${ORCH_ATTEMPT}\"}"
```
...
ORCH_WORKER_ID="$ORCH_WORKER_ID" python3 .claude/skills/orch-report/scripts/emit.py \
  ...
  --task-id "$ORCH_TASK_ID" \
  --attempt "$ORCH_ATTEMPT" \
  --data '{"phase":"test","reason":"emit_completed_failed","retryable":true}'
```
