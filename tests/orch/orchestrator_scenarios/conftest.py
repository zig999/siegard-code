...
def enter_phase(name, order, wf_id=WORKFLOW_ID):
    append_event("orchestrator", "phase_entered", data={"phase": name, "order": order, "workflow_id": wf_id})
...
def fail_task(task_id, phase, worker_id=None, retryable=False, attempt=1):
    ...
    append_event(wid, "task_failed", task_id=task_id, attempt=attempt, data={
        "phase": phase, "reason": "internal_error", "retryable": retryable,
    })
