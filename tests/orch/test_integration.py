...
def _declare_phase(name="dev", order=1):
    ...
    append_event("orchestrator", "phase_entered", data={"phase": name, "order": order, "workflow_id": "wf-fix"})
...
def _fail(task_id, attempt=1, retryable=True, phase="dev"):
    append_event("worker", "task_failed", task_id=task_id, attempt=attempt, data={
        "phase": phase, "reason": "stale_timeout", "retryable": retryable,
    })
...
class TestPhaseTransition:
    def test_tasks_promoted_after_phase_transition(self, tmp_orch):
        ...
        append_event("orchestrator", "phase_exit_approved", data={
            "phase": "sdd", "criteria_met": ["all_done"], "next_phase": "dev", "workflow_id": "wf-test",
        })
        append_event("orchestrator", "phase_transitioned", data={
            "from_phase": "sdd", "to_phase": "dev", "evidence_seq": 1, "workflow_id": "wf-test",
        })
        append_event("orchestrator", "phase_entered", data={"phase": "dev", "order": 2, "workflow_id": "wf-fix"})
