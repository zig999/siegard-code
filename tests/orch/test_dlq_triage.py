...
class TestBucketClassification:
    def test_spec_unclear_is_input_issue(self):
        assert dt._classify("validation_failed", None) == "input_issue"
...
class TestDlqTriageCLI:
    ...
    def _setup_dlq_task(self, cwd, task_id="t_001", reason="validation_failed", tier="standard"):
    ...
    def test_dlq_task_classified_correctly(self, tmp_path):
        self._setup_dlq_task(tmp_path, reason="validation_failed")
        r = subprocess.run(
            [sys.executable, TRIAGE, "--json"],
            cwd=str(tmp_path), capture_output=True, text=True
        )
    ...
    def test_critical_task_has_critical_prefix(self, tmp_path):
        self._setup_dlq_task(tmp_path, reason="validation_failed", tier="critical")
        r = subprocess.run(
            [sys.executable, TRIAGE, "--json"],
            cwd=str(tmp_path), capture_output=True, text=True
        )
